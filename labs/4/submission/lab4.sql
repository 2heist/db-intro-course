-- 1. Базова статистика: Середня ціна оренди та найдорожче авто
SELECT 
    COUNT(*) as total_cars,
    AVG(price_per_hour) as average_price,
    MAX(price_per_hour) as max_price,
    MIN(price_per_hour) as min_price
FROM car;

-- 2. Групування: Скільки машин кожного бренду є в автопарку
SELECT brand, COUNT(*) as car_count
FROM car
GROUP BY brand
ORDER BY car_count DESC;

-- 3. Групування + Сортування: Загальний дохід від кожного користувача
SELECT user_id, SUM(total_cost) as total_spent
FROM rent
WHERE status = 'completed'
GROUP BY user_id
ORDER BY total_spent DESC;

-- 4. Фільтрація груп (HAVING): Показати тільки ті локації, де стоїть більше 1 машини
SELECT location_id, COUNT(*) as cars_at_location
FROM car
GROUP BY location_id
HAVING COUNT(*) > 1;

-- 5. INNER JOIN: Детальна інформація про поїздки
SELECT 
    r.rent_id,
    u.name AS user_name,
    c.brand || ' ' || c.model AS car_name,
    r.total_cost,
    r.start_time
FROM rent r
INNER JOIN users u ON r.user_id = u.user_id
INNER JOIN car c ON r.car_id = c.car_id;

-- 6. LEFT JOIN: Статистика користувачів
-- Показує всіх юзерів, навіть якщо у них немає записів у таблиці rent
SELECT 
    u.name,
    COUNT(r.rent_id) as total_trips
FROM users u
LEFT JOIN rent r ON u.user_id = r.user_id
GROUP BY u.name;

-- 7. CROSS JOIN: Матриця можливих бронювань
-- Створює комбінацію "Кожен юзер" + "Кожна машина" 
SELECT 
    u.name as user,
    c.brand || ' ' || c.model as potential_car
FROM users u
CROSS JOIN car c
LIMIT 10;

-- 8. Підзапит у WHERE: Знайти машини, які дешевші за середню ціну по парку
SELECT brand, model, price_per_hour
FROM car
WHERE price_per_hour < (
    SELECT AVG(price_per_hour) FROM car
);

-- 9. Підзапит у SELECT: Вивести список машин і кількість відгуків про кожну
SELECT 
    brand, 
    model,
    (SELECT COUNT(*) FROM review WHERE review.car_id = car.car_id) as review_count
FROM car;

-- 10. Підзапит з IN: Знайти користувачів, які орендували машини бренду "Tesla"
SELECT name, email
FROM users
WHERE user_id IN (
    SELECT r.user_id 
    FROM rent r
    JOIN car c ON r.car_id = c.car_id
    WHERE c.brand = 'Tesla'
);